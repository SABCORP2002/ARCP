...
| `systemd/arcp-admin.service` | Service de l'administration (CMS) — `127.0.0.1:3001`, écrit dans `cms/data` + `cms/media` |
...
base serveur → secrets → `node scripts/deploy.mjs` → services systemd → DNS/HTTPS
→ contrôles de santé → sauvegardes.
